...
export const getUserAddressNum = (addr: string): Promise<Array<number>> => {
  return new Promise((resolve, reject) => {
    ...
    axios({
      ...
      url: `https://naveropenapi.apigw.ntruss.com/naver/map-geocode/v2/geocode?query=${addr}`,
    })
      .then((response) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        axios({
          ...
          url: `https://naveropenapi.apigw.ntruss.com/naver/map-reversegeocode/v2/gc?coords=${position.coords.longitude},${position.coords.latitude}&orders=legalcode&output=json`,
        })
          .then((response) => {
